...
/-!
# Transfinite announcements in BPALC
...
def ordinalUpdate (M : Model World Atom Agent) (φ : BPALCFormula Atom Agent)
    (α : Ordinal.{max u w}) : Model World Atom Agent :=
  { rel := fun i x y => M.rel i x y ∧ ∀ β, β < α → (ordinalUpdate M φ β).rSatisfies y φ
    val := M.val }
termination_by α
decreasing_by assumption

/-- Unfolding the relation does not require any frame hypothesis. -/
theorem ordinalUpdate_rel_iff (M : Model World Atom Agent) (φ : BPALCFormula Atom Agent)
    (α : Ordinal.{max u w}) (i : Agent) (x y : World) :
    (ordinalUpdate M φ α).rel i x y ↔
      M.rel i x y ∧ ∀ β < α, (ordinalUpdate M φ β).rSatisfies y φ := by
  rw [ordinalUpdate]

@[simp] theorem ordinalUpdate_val (M : Model World Atom Agent) (φ : BPALCFormula Atom Agent)
    (α : Ordinal.{max u w}) (p : Atom) (x : World) :
    (ordinalUpdate M φ α).val p x ↔ M.val p x := by
  rw [ordinalUpdate]

/-- The rich ordinal dynamics coincide with the semantics already defined in
`BPALC`, including formulas containing common belief and nested announcements. -/
theorem ordinalUpdate_eq_bpalcOrdinalIterate (M : Model World Atom Agent)
    (φ : BPALCFormula Atom Agent) (α : Ordinal.{max u w}) :
    ordinalUpdate M φ α = BPALC.ordinalIterate M φ α := by
  induction α using Ordinal.induction with
  | h α ih =>
    apply Model.ext'
    · intro i x y
      rw [ordinalUpdate_rel_iff, BPALC.ordinalIterate]
      ...
      rw [ih β hβ]
    · intro p x
      rw [ordinalUpdate_val, BPALC.ordinalIterate]

@[simp] theorem ordinalUpdate_zero (M : Model World Atom Agent) (φ : BPALCFormula Atom Agent) :
...
@[simp] theorem ordinalUpdate_succ (M : Model World Atom Agent) (φ : BPALCFormula Atom Agent)
    (α : Ordinal.{max u w}) :
    ordinalUpdate M φ (Order.succ α) = (ordinalUpdate M φ α).rUpdate φ := by
  apply Model.ext'
  · intro i x y
    rw [ordinalUpdate_rel_iff, Model.rUpdate_rel, ordinalUpdate_rel_iff]
    constructor
    · rintro ⟨hxy, h⟩
...
theorem ordinalUpdate_rel_antitone (M : Model World Atom Agent)
    (φ : BPALCFormula Atom Agent) {α β : Ordinal.{max u w}} (hαβ : α ≤ β)
    {i : Agent} {x y : World} :
...
theorem ordinalUpdate_limit_rel_iff (M : Model World Atom Agent)
    (φ : BPALCFormula Atom Agent) {α : Ordinal.{max u w}} (hα : IsSuccLimit α)
    (i : Agent) (x y : World) :
    (ordinalUpdate M φ α).rel i x y ↔
  ...
  · intro h
    ...
    · have hsucc := h (Order.succ β) (hα.succ_lt hβ)
      rw [ordinalUpdate_succ, Model.rUpdate_rel] at hsucc
      exact hsucc.2
...
@[simp] theorem ordinalUpdate_natCast (M : Model World Atom Agent)
    ...
    ordinalUpdate M φ (n : Ordinal.{max u w}) = M.rIterateUpdate φ n := by
  induction n with
  | zero => simp
  | succ n ih => rw [Ordinal.natCast_succ, ordinalUpdate_succ, ih, Model.rIterateUpdate_succ]
...
theorem ordinalUpdate_add (M : Model World Atom Agent) (φ : BPALCFormula Atom Agent)
    (α β : Ordinal.{max u w}) :
...
theorem ordinalUpdate_isK45 (M : Model World Atom Agent)
    (φ : BPALCFormula Atom Agent) (hM : IsK45 M) (α : Ordinal.{max u w}) :
...
theorem ordinalUpdate_eq_of_ge_of_step (M : Model World Atom Agent)
    (φ : BPALCFormula Atom Agent) {α β : Ordinal.{max u w}}
      ...
      · intro hxy
        ...
        · have heq := ih γ hγ (le_of_not_gt hγα)
          ...
          have harrow : (ordinalUpdate M φ (Order.succ α)).rel i x y := by
            ...
            exact hxy
          rw [ordinalUpdate_succ, Model.rUpdate_rel] at harrow
          exact harrow.2
    · intro p x
      simp
...
theorem common_ordinal_locallyAgrees (M : Model World Atom Agent)
    (φ : BPALCFormula Atom Agent) {α : Ordinal.{max u w}} {x : World}
...
theorem common_ordinal_stage_permanent (M : Model World Atom Agent)
    (φ : BPALCFormula Atom Agent) {α : Ordinal.{max u w}} {x : World}
    (hC : Common (ordinalUpdate M φ α) x φ)
    {β : Ordinal.{max u w}} (hαβ : α ≤ β) (ψ : BPALCFormula Atom Agent) :
    (ordinalUpdate M φ β).rSatisfies x ψ ↔ (ordinalUpdate M φ α).rSatisfies x ψ := by
  have hU : (ordinalUpdate M φ α).ForwardClosed
      ((ordinalUpdate M φ α).generatedSet x) :=
    (ordinalUpdate M φ α).generatedSet_forwardClosed x
  exact (BPALC.initiallySatisfies_iff_of_locallyAgrees hU
...
theorem common_ordinal_permanent {M : Model World Atom Agent} {x : World}
    {φ : BPALCFormula Atom Agent} (hC : Common M x φ)
    (α : Ordinal.{max u w}) (ψ : BPALCFormula Atom Agent) :
    (ordinalUpdate M φ α).rSatisfies x ψ ↔ M.rSatisfies x ψ := by
...
theorem common_of_update_eq_self (M : Model World Atom Agent)
    (φ : BPALCFormula Atom Agent) (h : M.rUpdate φ = M) : ∀ x, Common M x φ :=
  common_at_fixed h

/-- Proposition 3, including common belief at the fixed stage. -/
theorem ordinal_stabilization (M : Model World Atom Agent) (φ : BPALCFormula Atom Agent) :
    ∃ α : Ordinal.{max u w},
