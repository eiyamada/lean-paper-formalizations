...
/-!
# Finite eventuality, cofinal truth values, and convergence
...
eventuality: the converse implication requires compactness.
-/
...
variable {φ : Formula Atom Agent} {i j : Bool}

theorem holdsBit_not_iff (b : Bool) (P : Prop) :
    HoldsBit (!b) P ↔ ¬ HoldsBit b P := by
  cases b <;> simp [HoldsBit, Pattern.HoldsBit]

theorem holdsBit_congr (b : Bool) {P Q : Prop} (h : P ↔ Q) :
    HoldsBit b P ↔ HoldsBit b Q := by
  cases b <;> simp [HoldsBit, Pattern.HoldsBit, h]

/-- Restarting announcements at stage `m` shifts the trace by `m`. -/
theorem trace_shift {World : Type u} (M : Model World Atom Agent)
    (x : World) (φ : Formula Atom Agent) (m n : Nat) :
    (M.iterateUpdate φ m).trace x φ n ↔ M.trace x φ (m + n) := by
  unfold Model.trace
  rw [Model.iterateUpdate_add]
...
theorem strongEventual_iff_conditional_convergence :
    StrongEventual.{u} i j φ ↔
      ∀ {World : Type u} (M : Model World Atom Agent), IsK45 M → ∀ x,
        HoldsBit i (M.Satisfies x φ) → ConvergesTo j (M.trace x φ) := by
  constructor
...
theorem eventual_same_iff_cofinal :
    Eventual.{u} i i φ ↔
      ∀ {World : Type u} (M : Model World Atom Agent), IsK45 M → ∀ x,
        HoldsBit i (M.Satisfies x φ) → CofinalValue i (M.trace x φ) := by
  constructor
  · intro h World M hM x hx N
    ...
    | succ N ih =>
      ...
      obtain ⟨n, hn, hnext⟩ := h (M.iterateUpdate φ m)
        (M.iterateUpdate_isK45 hM φ m) x hvalue
...
theorem eventual_reversal_iff_cofinal :
    Eventual.{u} (!j) j φ ↔
      ∀ {World : Type u} (M : Model World Atom Agent), IsK45 M → ∀ x,
        CofinalValue j (M.trace x φ) := by
  classical
  constructor
  · intro h World M hM x N
    by_cases hv : HoldsBit j (M.trace x φ N)
    · exact ⟨N, le_rfl, hv⟩
    · have hstart : HoldsBit (!j) ((M.iterateUpdate φ N).Satisfies x φ) :=
        (holdsBit_not_iff j _).mpr hv
      obtain ⟨n, hn, hnext⟩ := h (M.iterateUpdate φ N)
        (M.iterateUpdate_isK45 hM φ N) x hstart
      exact ⟨N + n, by omega,
...
theorem strongEventual_reversal_iff_convergence :
    StrongEventual.{u} (!j) j φ ↔
      ∀ {World : Type u} (M : Model World Atom Agent), IsK45 M → ∀ x,
        ConvergesTo j (M.trace x φ) := by
  classical
  constructor
  · intro h World M hM x
    by_cases hex : ∃ m, HoldsBit (!j) (M.trace x φ m)
    · obtain ⟨m, hm⟩ := hex
      obtain ⟨N, _, htail⟩ := h (M.iterateUpdate φ m)
        (M.iterateUpdate_isK45 hM φ m) x hm
    ...
    · refine ⟨0, fun n _ => ?_⟩
      have hn : ¬ HoldsBit (!j) (M.trace x φ n) := fun hn => hex ⟨n, hn⟩
...
theorem eventual_iff_conditional_cofinal :
    Eventual.{u} i j φ ↔
      ∀ {World : Type u} (M : Model World Atom Agent), IsK45 M → ∀ x,
        HoldsBit i (M.Satisfies x φ) → CofinalValue j (M.trace x φ) := by
  constructor
  · intro h
    by_cases hij : i = j
    · subst j
...
theorem eventual_value_at_stable_stage (h : Eventual.{u} i j φ)
    ...
    (x : World) (hx : HoldsBit i (M.Satisfies x φ)) (N : Nat)
    (hstable : ∀ n, N ≤ n → (M.trace x φ n ↔ M.trace x φ N)) :
    HoldsBit j (M.trace x φ N) := by
...
def UnconditionalUniformBound (j : Bool) (φ : Formula Atom Agent) : Prop :=
  ∃ N : Nat, 1 ≤ N ∧
    ∀ {World : Type u} (M : Model World Atom Agent), IsK45 M → ∀ x,
...
theorem uniformBound_reversal_iff_unconditional :
  ...
  · rintro ⟨N, hN, hbound⟩
    ...
    by_cases hv : HoldsBit j (M.trace x φ 1)
    · exact ⟨1, le_rfl, by omega, hv⟩
    · obtain ⟨n, hn, hnN, hvalue⟩ := hbound (M.iterateUpdate φ 1)
        (M.iterateUpdate_isK45 hM φ 1) x ((holdsBit_not_iff j _).mpr hv)
      exact ⟨1 + n, by omega, by omega,
        (holdsBit_congr j (trace_shift M x φ 1 n)).mp hvalue⟩
  · rintro ⟨N, hN, hbound⟩
    exact ⟨N, hN, fun M hM x _ => hbound M hM x⟩

/-- Every agent's accessibility relation is empty. -/
def Edgeless {World : Type u} (M : Model World Atom Agent) : Prop :=
  ∀ a x y, ¬ M.rel a x y

/-- A uniform stage destroys all arrows and makes the formula false everywhere. -/
def UniformExtinction (φ : Formula Atom Agent) : Prop :=
  ∃ N : Nat, 1 ≤ N ∧
    ∀ {World : Type u} (M : Model World Atom Agent), IsK45 M →
      (∀ x, ¬ M.trace x φ N) ∧ Edgeless (M.iterateUpdate φ N)

theorem edgeless_update_eq {World : Type u} (M : Model World Atom Agent)
    (hM : Edgeless M) (φ : Formula Atom Agent) : M.update φ = M := by
  apply Model.ext'
  · intro a x y
...
theorem edgeless_iterateUpdate_eq {World : Type u} (M : Model World Atom Agent)
    (hM : Edgeless M) (φ : Formula Atom Agent) (n : Nat) :
    M.iterateUpdate φ n = M := by
  induction n with
  | zero => rfl
  | succ n ih => rw [Model.iterateUpdate_succ, ih, edgeless_update_eq M hM]
...
theorem uniformBound_self_refuting_implies_extinction
  ...
  have hedge : Edgeless (M.iterateUpdate φ (N + 1)) := by
    intro a x y hxy
    have hsurv := (M.iterateUpdate_rel_iff φ (N + 1) a x y).mp hxy |>.2
    have hy : HoldsBit true (M.Satisfies y φ) := hsurv 0 (by omega)
    obtain ⟨n, _, hnN, hn⟩ := hbound M hM y hy
    exact hn (hsurv n (by omega))
  refine ⟨fun x hx => ?_, hedge⟩
  obtain ⟨n, _, hn⟩ := hE (M.iterateUpdate φ (N + 1))
    (M.iterateUpdate_isK45 hM φ (N + 1)) x hx
  have heq := edgeless_iterateUpdate_eq (M.iterateUpdate φ (N + 1)) hedge φ n
  exact hn (by simpa only [Model.trace, heq] using hx)
...
theorem uniformExtinction_implies_strongEventual
  ...
  have heq : M.iterateUpdate φ n = M.iterateUpdate φ N := by
    rw [← Nat.add_sub_of_le hn, M.iterateUpdate_add]
    exact edgeless_iterateUpdate_eq _ hedge φ _
  simpa only [HoldsBit, Pattern.HoldsBit, Bool.false_eq_true, ↓reduceIte,
    Model.trace, heq] using hfalse x
