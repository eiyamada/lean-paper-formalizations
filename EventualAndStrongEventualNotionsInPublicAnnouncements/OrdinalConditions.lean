...
/-!
# Ordinal eventuality and common-belief conditions
...
def OrdinalEventual (i j : Bool) (φ : Formula Atom Agent) : Prop :=
  ∀ {World : Type u} (M : Model World Atom Agent), IsK45 M → ∀ x,
    HoldsBit i (M.Satisfies x φ) →
      ∃ α : Ordinal.{max u w}, 0 < α ∧
        HoldsBit j ((ordinalUpdate M φ α).Satisfies x φ)

/-- The required truth value persists from some positive ordinal onward. -/
def OrdinalStrongEventual (i j : Bool) (φ : Formula Atom Agent) : Prop :=
  ∀ {World : Type u} (M : Model World Atom Agent), IsK45 M → ∀ x,
    HoldsBit i (M.Satisfies x φ) →
      ∃ α : Ordinal.{max u w}, 0 < α ∧ ∀ β, α ≤ β →
        HoldsBit j ((ordinalUpdate M φ β).Satisfies x φ)

/-- The S condition at every ordinal stage. -/
def OrdinalStageCondition (i j : Bool) (φ : Formula Atom Agent) : Prop :=
  ∀ {World : Type u} (M : Model World Atom Agent), IsK45 M → ∀ x,
...
theorem ordinalEventual_same_implies_finiteStageCondition
  ...
  | h k ih =>
    ...
    · obtain ⟨n, hnk, rfl⟩ := exists_natCast_of_lt_natCast (lt_of_not_ge hkα)
      ...
      have hv' : HoldsBit i ((M.iterateUpdate φ n).Satisfies x φ) := by
        simpa only [ordinalUpdate_natCast] using hv
      have hCtail : Common ((M.iterateUpdate φ n).iterateUpdate φ (k - n)) x φ := by
        rw [← Model.iterateUpdate_add, hsum]
        exact hC
      have htail := ih (k - n) hnk' (M.iterateUpdate φ n)
        (M.iterateUpdate_isK45 hM φ n) x hv' hCtail
      change HoldsBit i (((M.iterateUpdate φ n).iterateUpdate φ (k - n)).Satisfies x φ)
        at htail
      rw [← Model.iterateUpdate_add, hsum] at htail
      exact htail
...
theorem ordinalEventual_reversal_implies_finiteStageCondition
  ...
  by_contra hv
  have hi : HoldsBit i ((M.iterateUpdate φ k).Satisfies x φ) := by
    simpa only [holdsBit_not_iff, not_not] using hv
  obtain ⟨α, _, hα⟩ := h (M.iterateUpdate φ k)
    (M.iterateUpdate_isK45 hM φ k) x hi
